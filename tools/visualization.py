...
def get_coords_color(opt):
    ...
    # same color order according to instance pointnum
        ...
        inst_class = np.zeros(rgb.shape[0]).astype(int)
        ...
        for i_ in range(len(masks) - 1, -1, -1):
            ...
            inst_label[mask == 1] = i
            inst_class[mask == 1] = masks[i][1]
        sort_idx = np.argsort(ins_pointnum)[::-1]
        inst_num.append(leaf_num)
        inst_num.append(stem_num)
        inst_num.append(ear_num)
        for _sort_id in range(ins_num):
    ...
    return xyz, rgb, inst_class, inst_label, inst_num


def write_ply(verts, colors, inst_classes, inst_labeles, inst_num, indices, output_file):
    if colors is None:
    ...
    # file.write('property uchar class\n')
    # file.write('property uchar inst\n')
    # file.write('element face {:d}\n'.format(len(indices)))
    # file.write('property list uchar uint vertex_indices\n')
    # file.write('end_header\n')
    file.write('{:d} {:d} {:d} 0 0 0 0 0\n'.format(inst_num[0], inst_num[1], inst_num[2]))
    for vert, color, inst_class, inst_label in zip(verts, colors, inst_classes, inst_labeles):
...
if __name__ == '__main__':
    ...
    xyz, rgb, inst_class, inst_label, inst_num= get_coords_color(opt)
    points = xyz[:, :3]
    colors = rgb / 255

    if opt.out != '':
        #assert '.ply' in opt.out, 'output cloud file should be in FILE.ply format'
        write_ply(points, colors, inst_class, inst_label, inst_num, None, opt.out)
    else:
